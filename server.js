...
app.post("/deposit", async (req, res) => {
  try {
    ...
    if (!stripeCustomerId || !accessToken) {
    ...
    console.error("Deposit error:", err.raw?.message || err.message);
    res.status(500).json({ error: "Deposit failed", details: err.raw?.message || err.message });
  }
});
